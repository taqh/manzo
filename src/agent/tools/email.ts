...
const MAX_PENDING_DRAFT_AGE_MS = 24 * 60 * 60 * 1000;
const DRAFT_INTENT_PATTERN =
  /\b(?:compose|draft|email|mail|reply|respond|send|write|tell(?:ing)?\s+(?:them|him|her)|say\s+(?:that|to))\b/i;
const ATTACHMENT_INTENT_PATTERN =
  /\b(?:show|send|share|forward|give|download|attach|include|use)\b[\s\S]{0,100}\b(?:attachment|attachments|file|files|document|documents|pdf|image|images|photo|photos|resume|cv|these|this)\b/i;
const ATTACHMENT_SHARE_INTENT_PATTERN =
  /\b(?:show|send|share|forward|give|download)\b[\s\S]{0,100}\b(?:attachment|attachments|file|files|document|documents|pdf|image|images|photo|photos)\b/i;
...
function hasAttachmentIntent(message: string): boolean {
  return ATTACHMENT_INTENT_PATTERN.test(message);
}

function hasAttachmentShareIntent(message: string): boolean {
  return ATTACHMENT_SHARE_INTENT_PATTERN.test(message);
}
...
function directSendGuard(
  ...
  if (input.authorization !== "telegram_allowlisted") {
    return {
      allowed: false,
      code: "blocked",
      reason: "Only the allowlisted Telegram chat can authorize sending.",
    };
  }
  if (!isExplicitDirectSendRequest(userMessage)) {
    return {
      ...
      code: "blocked",
      reason:
        "The current Telegram message did not explicitly authorize a direct send.",
    };
  }
  if (!hasUsableDirectSendContent(userMessage)) {
    return {
      ...
      code: "blocked",
      reason:
        "The current Telegram message does not contain usable message content.",
    };
  }
  if (
    recipient &&
    ...
    return {
      allowed: false,
      code: "blocked",
      reason: "The recipient was not present in the current Telegram message.",
    };
  }
  if (draftLeaksUnmentionedMemory(body, userMessage, memories)) {
    return {
      ...
      code: "blocked",
      reason:
        "The outbound text included personal memory not mentioned in the current request.",
    };
  }
  if (!directSendBodyMatches(userMessage, body)) {
    return {
      ...
      code: "body_mismatch",
      reason:
        "The generated body did not match the content authorized in the current message.",
    };
  }
  return { allowed: true };
}
...
async function sendAndRecord(
  ...
  try {
    return recordSent(context, await context.host.sendDraft(draftId, revision));
  } catch (error) {
  ...
  return {
    checkPreviousCorrespondence: tool({
      ...
      execute: ({ emailId }) => {
        ...
        if (!email) {
          return { emailId: reference, found: false };
        }
    ...
    createDraft: tool({
      description:
        "Create and preview, but do not send, a reply to an incoming email.",
      execute: ({ emailId, body, attachmentIds }) => {
        if (!hasDraftIntent(userMessage)) {
          return {
            created: false,
            reason:
              "The current Telegram message did not request a draft or reply.",
          };
        }
        ...
        if (!reference) {
          return {
            created: false,
            reason: "There is no selected or recently presented email.",
          };
        }
        if (draftLeaksUnmentionedMemory(body, userMessage, memories)) {
          return {
            created: false,
            reason:
              "The draft included personal memory not mentioned in the current request.",
          };
        }
        ...
        if (resolvedAttachmentIds === null) {
          return {
            created: false,
            reason:
              "That attachment is not one of the files recently uploaded in this Telegram chat.",
          };
        }
    ...
    createNewEmailDraft: tool({
      description:
        "Create and preview, but do not send, a brand-new email from the configured default mailbox.",
      execute: ({ recipient, subject, body, attachmentIds }) => {
        if (!hasDraftIntent(userMessage)) {
          return {
            created: false,
            reason:
              "The current Telegram message did not request a draft or email.",
          };
        }
        if (draftLeaksUnmentionedMemory(body, userMessage, memories)) {
          return {
    ...
    findPreviousEmailsFrom: tool({
      ...
      execute: ({ sender, excludeEmailId, limit }) => {
        const emails = host.findPreviousEmailsFrom(
          sender,
          excludeEmailId,
          limit
        );
        ...
        return {
          emails: summariesForModel(emails),
          security: "UNTRUSTED_EMAIL_METADATA",
        };
      },
      inputSchema: z.object({
    ...
    listEmails: tool({
      description:
        "List the owner's most recent stored emails. Returns untrusted metadata only.",
      execute: ({ limit }) => {
    ...
    sendAttachmentsToTelegram: tool({
      ...
      execute: async ({ emailId, attachmentIds }) => {
        ...
        const email = host.readEmail(reference);
        if (!email) {
          return { emailId: reference, shared: false };
        }
    ...
    sendNewEmail: tool({
      ...
      execute: async ({ recipient, subject, body, attachmentIds }) => {
        ...
        if (
          ...
          return {
            reason:
              "The current Telegram message did not explicitly ask to attach a file.",
            sent: false,
          };
        }
        ...
        if (!guard.allowed) {
          if (guard.code === "body_mismatch") {
            ...
            return {
              previewCreated: true,
              reason:
                "The generated wording did not exactly match the authorized text, so it was previewed instead of sent.",
              sent: false,
            };
          }
          return { reason: guard.reason, sent: false };
        }
        void subject;
        const draft = host.createNewEmailDraft(
          recipient,
        ...
        if (!sent) {
          return { deliveryOutcome: "unknown", sent: false };
        }
        return { draftId: sent.draftId, messageId: sent.messageId, sent: true };
      },
      inputSchema: z.object({
        attachmentIds: z.array(z.string().min(1).max(128)).max(20).optional(),
        body: z.string().min(1).max(20_000),
    ...
    sendPendingDraft: tool({
      ...
      execute: async () => {
        const { pendingDraft } = input;
        if (!pendingDraft) {
          return {
            reason: "There is no displayed pending draft.",
            sent: false,
          };
        }
        ...
        if (pendingAge < 0 || pendingAge > MAX_PENDING_DRAFT_AGE_MS) {
          return {
            reason: "The displayed draft is too old; review a fresh draft.",
            sent: false,
          };
        }
        if (
          ...
          return {
            reason:
              "The current Telegram message did not explicitly confirm this draft.",
            sent: false,
          };
        }
        const sent = await sendAndRecord(
    ...
    sendReply: tool({
      ...
      execute: async ({ emailId, body, attachmentIds }) => {
        ...
        if (!reference) {
          return {
            reason: "There is no selected email to reply to.",
            sent: false,
          };
        }
        const draft = host.createDraft(reference, body, resolvedAttachmentIds);
